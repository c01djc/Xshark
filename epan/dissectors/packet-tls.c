...
/* Helper protocol so TLCP (GM/T 0024) can be filtered with "tlcp". */
static int proto_tlcp;
 ...
 *
 */
/* Mark frame as TLCP so the "tlcp" display filter matches. */
static void
tls_add_tlcp_protocol(proto_tree *tree, tvbuff_t *tvb, packet_info *pinfo)
{
    proto_item *ti;
    ...
    /* Only add once per frame (a frame may contain multiple records). */
    if (p_get_proto_data(pinfo->pool, pinfo, proto_tlcp, 0)) {
        return;
    }
    p_add_proto_data(pinfo->pool, pinfo, proto_tlcp, 0, GINT_TO_POINTER(1));

    ti = proto_tree_add_item(tree, proto_tlcp, tvb, 0, -1, ENC_NA);
                    ...
                    uint8_t curr_layer_num_ssl, struct tlsinfo *tlsinfo)
{
    ...
    /* Expose TLCP as its own display-filter protocol (in addition to tls). */
    if (version == TLCPV1_VERSION || record_version == TLCPV1_VERSION) {
        tls_add_tlcp_protocol(tree, tvb, pinfo);
    }
...
proto_register_tls(void)
{
    ...
    proto_tls = proto_register_protocol("Transport Layer Security",
                                        "TLS", "tls");
    /* TLCP (GM/T 0024) shares the TLS dissector; register a helper protocol
     * so display filters can use "tlcp" independently of "tls". */
    proto_tlcp = proto_register_protocol_in_name_only(
        "Transport Layer Cryptography Protocol",
        "TLCP", "tlcp", proto_tls, FT_PROTOCOL);
