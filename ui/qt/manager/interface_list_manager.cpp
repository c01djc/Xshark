...
static bool
capture_iface_auto_hidden(const char *name, const if_info_t *if_info)
{
    if (prefs_is_capture_device_hidden(name)) {
        return true;
    }

    const char *label = name;
#ifdef _WIN32
    if (if_info && if_info->friendly_name) {
        label = if_info->friendly_name;
    }
#endif
    if (!label) {
        return false;
    ...
    QString iface_label = QString::fromUtf8(label);
    static const QStringList auto_hidden_patterns = {
    ...
    for (const QString &pattern : auto_hidden_patterns) {
        if (iface_label.contains(pattern, Qt::CaseInsensitive)) {
            return true;
        }
    }
    return false;
...
scan_local_interfaces_filtered(capture_options* capture_opts, GList * allowed_types, void (*update_cb)(void))
{
    ...
    for (if_entry = if_list; if_entry != NULL; if_entry = g_list_next(if_entry)) {
        ...
        if (!found) {
            ...
            device.hidden = capture_iface_auto_hidden(if_info->name, if_info);
        ...
        bool selected = fill_from_ifaces(capture_opts, &device);
...
update_local_interfaces(capture_options* capture_opts)
{
    ...
    for (i = 0; i < capture_opts->all_ifaces->len; i++) {
        ...
        g_free (descr);
        device->hidden = capture_iface_auto_hidden(device->name, &device->if_info);
