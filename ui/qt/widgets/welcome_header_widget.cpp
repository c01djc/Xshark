    ...
    header_ui_(new Ui::WelcomeHeaderWidget)
{
    ...
    /* Product version for XShark (independent of upstream Wireshark VERSION). */
    header_ui_->headerVersion->setText(QStringLiteral("V1.0.0"));
    header_ui_->headerBuildLabel->setText(tr("Wireshark 国密专版"));
    header_ui_->headerBuildLabel->show();

    header_ui_->updateHeader->setObjectName(QStringLiteral("gmBrandHeader"));
    header_ui_->updateHeader->setVisible(true);
    header_ui_->updateDot->hide();
    header_ui_->updateReleaseNotes->hide();
    header_ui_->updateDownload->hide();
    header_ui_->updateDismiss->hide();
    header_ui_->updateText->setObjectName(QStringLiteral("gmBrandTitle"));
    header_ui_->updateText->setText(tr("Wireshark 国密专版 · XShark（小鲨鱼）  |  独立 tlcp 显示过滤，不与普通 TLS 混筛 · 国密密评专用"));
#else
