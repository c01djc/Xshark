...
recent_read_static(char **rf_path_return, int *rf_errno_return)
{
    ...
    *rf_path_return = NULL;
    if ((rf = ws_fopen(rf_path, "r")) != NULL) {
        ...
        fclose(rf);
#ifdef GM_WIRESHARK_BUILD
        /* GM builds always hide the promotional welcome sidebar, even when
         * recent_common was created by stock Wireshark with sidebar enabled. */
        recent.gui_welcome_page_sidebar_learn_visible = false;
        recent.gui_welcome_page_sidebar_tips_visible = false;
        recent.gui_welcome_page_sidebar_tips_events = false;
        recent.gui_welcome_page_sidebar_tips_sponsorship = false;
        recent.gui_welcome_page_sidebar_tips_tips = false;
        if (!recent.gui_theme_name || strcmp(recent.gui_theme_name, "wireshark") == 0) {
            g_free(recent.gui_theme_name);
            recent.gui_theme_name = g_strdup("gm-wireshark");
        }
#endif
